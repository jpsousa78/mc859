...
Compras_Entrada::Compras_Entrada(string filename) : vname(this->g),
                                                    px(this->g),
                                                    py(this->g),
                                                    dist(this->g),
                                                    fatorpeso(this->g),
                                                    preco(this->g)
{
  ...
  for (int i = 0; i < this->narcs; i++)
  {
    ...
    for (DNodeIt u(this->g); u != INVALID; ++u)
      if (this->vname[u] == v_origem[i])
      {
        origem = u;
        break;
      }
    for (DNodeIt u(this->g); u != INVALID; ++u)
      if (this->vname[u] == v_destino[i])
      {
        destino = u;
        break;
      }
...
Compras_Entrada::Compras_Entrada(void) : vname(this->g),
                                         px(this->g),
                                         py(this->g),
                                         dist(this->g),
                                         fatorpeso(this->g),
...
bool Compras_Entrada::verificasolucao(void)
{
  ...
  cout << "Deposito: " << this->vname[this->Deposito] << endl
       << endl;
  if (this->nnodesequence < 2)
  {
    cout << "Erro: Solucao deve ter pelo menos dois no's." << endl;
    return (false);
  }
  ...
  {
    cout << "Erro: Ultimo no' da solucao deve ser o deposito." << endl;
    return (false);
  }
  for (DNodeIt v(this->g); v != INVALID; ++v)
    Visitado[v] = false;
  for (int j = 0; j < this->nnodesequence - 1; j++)
  {
    ...
    if (!ok)
    {
      cout << "Erro: Nao encontrou arco da loja " << this->vname[nodesequence[j]] << " para a loja " << this->vname[nodesequence[j + 1]] << "." << endl;
      return (false);
    }
  }
  for (int i = 0; i < this->nitens; i++)
  {
    ...
    {
      cout << "Erro: Loja do item " << i << " nao esta' na solucao." << endl;
      return (false);
    }
  }
  return (true);
...
bool Compras_Entrada::visualizasolucao(void)
{
  ...
  for (DNodeIt v(this->g); v != INVALID; ++v)
    G.SetColor(v, "Gray"); // vertices em cinza.
  for (ArcIt a(this->g); a != INVALID; ++a)
    G.SetColor(a, "Invis"); // todos arcos de invisivel
